...
.. module:: map_lucid.py
   ...
   :synopsis: Maps LUCID ESGF publisher ini file to normalized pyessv format.
...
def yield_las_time_delta(ctx):
    ...
    for time_frequency, las_time_delta in ctx.ini_section.get_option('las_time_delta_map', '\n', '|'):
        src_namespace = 'wcrp:lucid:time-frequency:{}'.format(time_frequency.lower().replace('_', '-'))
...
def yield_institute(ctx):
    ...
    for model, institute in ctx.ini_section.get_option('institute_map', '\n', '|'):
        src_namespace = 'wcrp:lucid:model:{}'.format(model.lower().replace('_', '-'))
